...
- Aba MATERIAIS = TODO material de instalação num só lugar: tubulação, suporte/mão-francesa, dreno, canaleta, fita, isolamento, gás/carga, E TAMBÉM o elétrico (fio/cabo, disjuntor, tomada, protetor). Tudo que é categoria:"material" cai aqui.
- Aba SERVIÇOS = mão de obra: instalação, limpeza, visita técnica, manutenção, PMOC, recarga.
Quando o técnico manda adicionar/remover algo, você NÃO precisa saber de "qual aba" — você só classifica certo pela "categoria" ("material" ou "servico") e o app coloca na aba certa sozinho. Ao REMOVER, mande a palavra-chave do item; o app tira de onde ele estiver (seja gerado pela instalação ou avulso).
...
export async function POST(req: NextRequest) {
  const { mensagem, historico, model, estadoAtual } = await req.json() as {
    ...
    estadoAtual?: {
      ...
      itensNoOrcamento?: { descricao: string; quantidade: number; unidade: string; categoria: string }[]
      clienteNome: string
  ...
  const contextoOrcamento = (temEquip || temExtras)
    ? [
        ...
        // Lista COMPLETA de itens que estão no orçamento agora (inclui materiais gerados
        // pela instalação). É esta lista que o técnico vê e sobre a qual pede remoções.
        (estadoAtual!.itensNoOrcamento && estadoAtual!.itensNoOrcamento.length > 0)
          ? 'ITENS ATUALMENTE NO ORÇAMENTO (é sobre ESTES que o técnico pede para tirar/remover):'
          : '',
        (estadoAtual!.itensNoOrcamento && estadoAtual!.itensNoOrcamento.length > 0)
          ? estadoAtual!.itensNoOrcamento.map(it =>
              `  - ${it.descricao} × ${it.quantidade}${it.unidade} (${it.categoria})`
            ).join('\n')
          : '',
        ...
        '- "remove/tira o 2" (um EQUIPAMENTO por índice) → use "atualizar_orcamento" e retorne a lista SEM o equipamento removido.',
        '- REMOVER UM MATERIAL/SERVIÇO (ex: "tira o disjuntor", "não precisa do suporte", "remove a tubulação extra"): use acao "adicionar" e itensExtras com {descricao:"<palavra-chave do item>", _op:"remove"}. Isso FUNCIONA mesmo que o item tenha sido gerado automaticamente pela instalação — o app o encontra pela palavra-chave e o tira da lista. NUNCA responda que removeu sem colocar o item em itensExtras com _op:"remove".',
        '- IMPORTANTE: se o item que o técnico quer tirar aparece na lista "ITENS ATUALMENTE NO ORÇAMENTO" acima, ele EXISTE e VOCÊ CONSEGUE removê-lo. Sempre emita o _op:"remove" — não diga que não encontrou.',
