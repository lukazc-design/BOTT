...
export function AuthForm({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  ...
  // Ao entrar no modo "Criar conta", leva o usuário direto para a região de
  // preenchimento dos dados (o formulário), sem precisar rolar a página.
  useEffect(() => {
    if (isLogin) return
    const t = setTimeout(() => {
      document.getElementById('form-acesso')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }, 250)
    return () => clearTimeout(t)
  }, [isLogin])

  // Posts do Instagram (código do post em instagram.com/p/CODIGO)
  const videos = [
    { id: 'DbsXKWlNtid', titulo: 'Na prática: fazendo um orçamento' },
    { id: 'DbsXWOntq8T', titulo: 'Configuração do app' },
    { id: 'DbsXpF2Rsve', titulo: 'Ganhe tempo: apresentação da IA' },
  ]
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {videos.map((v, i) => (
              <div key={i} className="space-y-2">
                <div className="relative w-full overflow-hidden rounded-xl bg-muted" style={{ aspectRatio: '9 / 13' }}>
                  <iframe
                ...
                <a
                  href={`https://www.instagram.com/p/${v.id}/`}
                  target="_blank"
