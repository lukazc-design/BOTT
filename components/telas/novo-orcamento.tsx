...
export function NovoOrcamento({
  ...
  const enviarMensagemTexto = async (texto: string) => {
    ...
    const estadoAtual = (equipamentos.length > 0 || itensExtras.length > 0) ? {
      ...
      // TODOS os itens que aparecem no orçamento AGORA (inclui os materiais/serviços
      // gerados automaticamente pela instalação). A IA precisa VER isto para conseguir
      // remover corretamente pelo nome quando o técnico pede "tira o disjuntor" etc.
      itensNoOrcamento: itens
        .filter(it => it.id !== 'desconto-global')
        .map(it => ({ descricao: it.descricao, quantidade: it.quantidade, unidade: it.unidade, categoria: it.categoria })),
      clienteNome,
      clienteEndereco,
      clienteTelefone,
    } : null
  ...
  // Botão "IA" da barra inferior: fecha preview/drawer e volta direto ao chat da IA
  useEffect(() => {
    if (!focarChatNonce) return
    setMostrarPreview(false)
...
function PainelLateral({
  ...
  const cor = perfil.corPrimaria ?? '#0ea5e9'
  const [aba, setAba] = useState<'equipamentos' | 'materiais' | 'servicos' | 'preview'>('equipamentos')

  // "Materiais" reúne TODO material de instalação: tubulação, suporte, dreno, gás, fio, disjuntor, etc.
  const itensMateriais = itens.filter(i => i.categoria === 'material')
  ...
  type AbaId = 'equipamentos' | 'materiais' | 'servicos' | 'preview'
  const ABAS: { id: AbaId; label: string; icon: React.ReactNode; count?: number; soComOrcamento?: boolean }[] = [
    ...
    { id: 'materiais', label: 'Materiais', icon: <Package size={11} />, count: itensMateriais.length },
            ...
            {aba === 'materiais' && (
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[9px] text-muted-foreground uppercase tracking-wide">Materiais de instalacao (tubulacao, suporte, dreno, gas, fio, disjuntor...)</p>
                  {itensMateriais.length > 0 && (
                    <button
                      onClick={() => { if (confirm(`Apagar todos os ${itensMateriais.length} materiais da lista?`)) onRemoverItens(itensMateriais) }}
                ...
                {itensMateriais.length === 0
                  ? <p className="text-xs text-muted-foreground text-center py-4">Nenhum material calculado ainda</p>
                  : itensMateriais.map((it, i) => (
